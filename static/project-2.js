...
document.addEventListener('DOMContentLoaded', function()
{
    ...
    let workoutCount = 0;
    // Check to ensure Activity HTML page is triggering the event by checking if add workout button exists (prevents JavaScript errors on other pages)
    if (addWorkoutBtn){
    ...
    // Check to ensure activity page is triggering the event by checking if add workout button exists
    if (addWellnessBtn){
        ...
        if (napYesCheckbox){
                ...
                document.querySelector('form').addEventListener('change', function(event) {
                        ...
                        // Check if number of awakenings was target and is greater than 0
                        if (event.target === timesWokenUpInput && timesWokenUpInput.value > 0) {
                                timesWokenUpRow.style.display = 'table-row';
                        }

                        // Check if number of awakenings was target and is 0
                        else if (event.target === timesWokenUpInput && parseInt(timesWokenUpInput.value) === 0) {
                                timesWokenUpRow.style.display = 'none';
                        }
                });         
